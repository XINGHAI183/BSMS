...
Q: 如何查看站点的详细信息？
A: 在地图上点击任意一个车站节点，会弹出包含站名和换乘信息的气泡。具体实现可见 beijing-subway-map-super.html 中的 `showStationInfo` 函数。

Q: 如何搜索站点并快速定位？
A: 在“工具”区域上方“搜索站点”输入框输入名称，可模糊匹配，北京地铁所有站点均可搜，点击搜索结果即可让地图跳转至目标站点，并弹出该站点详情。

Q: 如何路径查询与模式切换？
A: 输入起点和终点后，点击“查询”按钮（`id="searchRouteBtn"`），下方会展示最优路线。上方有“向左/向右箭头”按钮（`id="toggleRouteModeBtnLeft/Right"`），分别切换“站数优先”或“换乘次数优先”模式，支持不同出行偏好。
